...
만일 *Y = (y, y')*로 정의하면, 이 방정식은 *Y' = f(t, Y)*의 형태로 쓸 수 있는 것이다. 그러므로 우리는 방정식을 풀어주는 모듈에 인자로 넘겨줄 함수는 다음과 같은 식으로 구성해야 할 것이다.
{% highlight C %}
...
{% highlight C %}
    ...
    ...
{% endhighlight %}
만일 이게 Fortran이었다면 코드가 보다 직관적이었을 것이다. 하지만 C에서는 그게 안 되기 때문에, 저렇게 임시변수의 포인터를 넘겨서 계산하는 삽질이 필요하다. 이제 마무리를 해주도록 하자. 사용한 메모리를 반납하면 함수는 끝이 난다.
{% highlight C %}
    ...
...
## 돌려보기
이제 미분방정식을 푸는 모듈을 만들었다. 실제로 돌려볼 시간이다. 우선 근의 공식이 존재하는 *y' = 2y* 부터 풀어보자. 너무 큰 범위에서 풀면 힘들테니, 가볍게 *t = 0 ~ 3* 정도까지 해보자.
{% highlight C %}
